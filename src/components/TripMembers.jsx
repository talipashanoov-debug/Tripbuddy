    ...
    <div className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {members.map((m) => {
            ...
            return (
              <span
                key={m.user_id}
                className="inline-flex items-center gap-1.5 rounded-full bg-slate-50 py-1 pl-1 pr-3 text-sm ring-1 ring-slate-200"
                title={owner ? 'Trip creator' : undefined}
              >
      ...
      {!loading && isCreator && (
        <form onSubmit={invite} className="mt-4 border-t border-slate-100 pt-4">
          <label className="mb-1 block text-sm font-medium text-slate-700">Invite a member</label>
          ...
          <p className="mt-2 text-xs text-slate-400">
            The person must already have a TripBuddy account.
          </p>
        </form>
      )}
    </div>
