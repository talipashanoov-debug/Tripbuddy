...
import { ArrowLeft, HandCoins, Sparkles } from 'lucide-react'
import { invokeFunction } from '../lib/invokeFunction'
import { formatCurrency } from '../lib/formatCurrency'
import { t } from '../lib/strings'

// Calls the `calculate-settlement` Edge Function. invokeFunction attaches the
// user's access token automatically, so the function can authorize the caller.
export default function SettlementPlan({ tripId }) {
  ...
  const calculate = async () => {
    setLoading(true)
    setError('')
    const { data, error } = await invokeFunction('calculate-settlement', { trip_id: tripId })
    ...
    <div className="mb-6 rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-semibold text-slate-800">{t.settlement.heading}</h3>
        </div>
        <button
          onClick={calculate}
          disabled={loading}
      ...
      {!result && !error && <p className="mt-3 text-sm text-slate-500">{t.settlement.intro}</p>}

      {result && (
        <div className="mt-4">
          <p className="text-xs text-slate-400">{t.settlement.summary(result.participant_count)}</p>
              ...
              {result.settlements.map((s, idx) => (
                ...
                >
                  <div className="flex items-center gap-2 text-sm">
                    <span className="font-medium text-slate-700">{s.from_name}</span>
                    <ArrowLeft className="h-4 w-4 shrink-0 text-slate-400" />
                    <span className="font-medium text-slate-700">{s.to_name}</span>
                  </div>
                  <span className="font-semibold text-emerald-600">{formatCurrency(s.amount)}</span>
                </li>
              ))}
            </ul>
          )}
